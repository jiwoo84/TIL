...
- **만약 프라미스 사용하지 않는다면?**
    ...
    위 과정에서 콜백 함수를 중첩해서 불러오고, 각각 다 구현하면 콜백지옥이 생성되며 오류를 일으킬 확률↑
...
- 성공/실패 여부에 따라 결과값 받아 실행
- error객체를 받아서 에러도 처리 가능
    
    → 이렇게 하면 then에서 발생한 에러는 처리 불가 (뒤에 catch 있다면 거기로 넘어감)
    
- 반환값이 promise라서 체이닝 가능
...
`async`이 붙은 함수는 프라미스를 반환하고, 프라미스가 아닌 것은 프라미스로 감싸 반환함
