...
3. `Object.entried(obj-name)`  [key, value] 쌍을 담은 배열 반환
...
object의 기본 틀(형식, 모양)을 만드는 함수의 일종
...
```jsx
class className {
  constructor(param, param...) { prop : ~; ... } // 1. 객체의 기본 형태 설정
...
1. **constructor** : 클래스로 객체가 생성 → 해당 함수가 실행 → 객체의 형태 만들어짐
...
- extends
    ...
    이렇게만 입력해도 basic-class의 프로퍼티 다 포함되어 있음
    
    - 기존 함수 재정의 가능 (덮어쓰기)
        
        ```jsx
        fnc-name() {
        	super.fnc-name(); // 부모의 메서드 호출 => 기존의 함수 가져옴
        	~; // 추가되는 내용
        }
        ```
